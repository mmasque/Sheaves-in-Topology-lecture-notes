...
\begin{defn}[Stalks]
Let $i_x\colon \{x\} \hookrightarrow X$ be the inclusion of a point in a space. Then $i_x^* \mathcal{F}$ is the \emph{stalk} $\mathcal{F}_x$ of $\mathcal{F}$ at $x$. 
Unravelling definitions, we have \[
    \mathcal{F}_x = \colim_{U \ni x} \mathcal{F}(U).
\]
An element of the stalk at $x$ is represented by a \emph{germ} $(s)_U$ for $U \ni x$ and $s \in \mathcal{F}(U)$, where \[
    (s)_U = (t)_V \text{ if and only if } \exists x \in W \subset U \cap V
\] such that $s|_{W} = t|_{W}$. 
\end{defn}
Note that the stalk $\mathcal{F}_x$ is the fibre of $\etalespace(X) \to X$ above $x$. 
