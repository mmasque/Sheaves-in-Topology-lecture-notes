...
\begin{defn}
The \emph{pullback}\index{pullback!of sheaves} \(f^*\mathcal F\) of a sheaf \(\mathcal F\) on \(X\) along \(f\) is the sheaf
...
\begin{lem}\label{lem:pushforward-pullback-adjunction-sheaves}
...
\begin{defn}
    Let $\{x\} \to X$ be the inclusion of a point $x \in X$. Let $S \in \catSet$. The \indexemph{skyscraper sheaf} at $x$ with value $S$ is $i_{x,*}\underline{S}$, the pullback of the constant sheaf. We denote it $i_{*, S}$.
\end{defn}
