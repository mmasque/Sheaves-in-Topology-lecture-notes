...
\begin{prop}
	...
	\begin{enumerate}
		...
		\item $f \times \id_W: Y \times W \to X \times W$ is closed,
		\item $f$ is closed and $f^{-1}(x)$ is closed for all $x \in X$,
...
\begin{defn}
	The $i$th \emph{higher pushforward with proper support} are the derived functors $\rightderived^if_!: \catAbelianSheaf(Y) \to \catAbelianSheaf(X)$ of the \emph{proper pushforward}. 
\end{defn}
...
\begin{proof}
	...
	Note that the pullback $i^*: \catAbelianSheaf(X) \to \catAbelianSheaf(Z)$ preserves soft sheaves if $X$ is paracompact Hausdorff (respectively c-soft sheaves if $X$ is locally compact Hausdorff, irrespective of whether $Z$ is compact). 
\end{proof}
