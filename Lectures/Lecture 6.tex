...
\begin{itemize}[resume]
    ...
    \item Sheaves of categories (a.k.a. stacks in categories)
...
\begin{defn}
    Let $\cat C$ be a category. A \emph{presheaf on $X\in\catTopologicalSpace$ with values in $\cat C$} is a functor $F\colon \open(X)\opp \to \cat C$. If $\cat C$ is complete (can weaken to $\cat C$ has products (why?)), then a sheaf on $X$ with values in $\cat C$ is a presheaf such that the sheaf condition holds in $\cat C$. 
...
\begin{defn}
    ...
    \begin{enumerate}
        ...
        \item[] (Commutativity) 
            \[\begin{tikzcd}
            A\times A \arrow[rd, "m"] \arrow[rr, "\text{swap}"] &   & A\times A \arrow[ld, "m"'] \\ 
            & A & 
            \end{tikzcd}\]
        \item[] (Identity)
            \[\begin{tikzcd}
            A\times * \arrow[r, "\id\times 0"] \arrow[d, "\text{pr}_1"] & A\times A \arrow[d, "m"] & *\times A \arrow[l, "0\times\id"'] \arrow[d, "\text{pr}_2"] \\
        ...
        \item[] (Inverse)
            ...
            * \arrow[d, "0"] &                           & A \arrow[d, "\Delta"] \arrow[rr] \arrow[ll]                           &                          & * \arrow[d, "0"] \\
            ...
            \end{tikzcd}\]
    \end{enumerate}
    Write $\catAbelianGroup(\cat C)$ for the \emph{category of abelian group objects} in $\cat C$. The morphisms of this category are
    \[\catAbelianGroup(\cat C)((A,m,i,0), (B, m,i, 0)) = \{f\in \cat C(A,B)\colon f\circ m = m\circ\left(f \times f\right)\}\] that is, those morphisms that commute with the group operation $m$. 
\end{defn}

\begin{rmk}
    $\catAbelianGroup(\catSet) = \catAbelianGroup$.
\end{rmk}

\begin{lem} We have the following equivalences of categories:
    \begin{enumerate}
        \item[(i)] $\catAbelianGroup(\catPresheaf(X)) \cong \catAbelianPresheaf(X)$
        \item[(ii)] $\catAbelianGroup(\catSheaf(X)) \cong \catAbelianSheaf(X)$
    \end{enumerate}
\end{lem}

\begin{proof}[sketch]
    \begin{enumerate}
        \item[(i)] A presheaf $F\colon \open(X)\opp \to \catAbelianGroup$ consists of $F(U)\in\catAbelianGroup$ for all $U\in \open(X)\opp$ with restriction maps $r_{UV}\colon F(V)\to F(U)$ in $\catAbelianGroup$. This means that the group operation $F(U)\times F(U)\to F(U)$ is natural, that is, if $U\subseteq V$ then
        \[\begin{tikzcd}
        ...
        \item[(ii)] We claimed earlier that $F\colon \open(X)\opp\to \catAbelianGroup$ is a sheaf if and only if $U\circ F\colon \open(X)\opp \to \catSet$ is a sheaf (where $U\colon \catAbelianGroup\to \catSet$ is the forgetful functor). Thus for any sheaf $\cat F\in \catAbelianSheaf(X)$, $U\circ \cat F$ is a sheaf of sets which has abelian group object structure by (i). Conversely, $(\mathcal F, m,i, 0)\in \catAbelianGroup(\catSheaf)$ is the data of a sheaf of abelian groups, since it is a presheaf of abelian groups by (i) and thus a sheaf since it is a sheaf on sets.  \qedhere
    ...
    \begin{itemize}
        \item We have an adjunction:
         ...
         which restricts to an equivalence:
          ...
          \end{tikzcd}\]
          \item Monodromy: $\catAbelianGroup(\catLocallyConstantSheaf(X)) \cong \catName{Rep}_{\bb Z}(\Pi_1(X,x))$ (where $\catName{Rep}_{\bb Z}$ denotes the category of representations in $\catModule_{\bb Z}$). We may also denote $\catAbelianGroup(\catLocallyConstantSheaf(X))$ by $\catAbelianGroup^{\text{l.c.}}(X) \cong \catModule_{\bb Z[\Pi_1(X,x)]}$.
...
Finally, since the image presheaf is not a sheaf we define exactness in the $\catAbelianGroup(X)$ via the stalks.

\begin{defn}
    A sequence $\cat F\xrightarrow{f}\cat G\xrightarrow{g}\cat H$ in $\catAbelianGroup(X)$ is exact at $\cat G$ iff $\cat F_x\xrightarrow{f_x} \cat G_x \xrightarrow{g_x} \cat H_x$ is exact for all $x\in X$.
\end{defn}

(Remark Remy in lecture: Coincidentally, the reason of this definition, that the image presheaf is not a sheaf, is also the reason the global sections functor (evaluation in the global sections) is generally only left exact, thus a right derived functor. Right deriving the global sections functor on a sheaf  $\cat F\in \catAbelianGroup(X)$ allows us to compute the sheaf cohomology denoted $H^i(X;\cat F)$.)

\begin{exmp}
    Let $0\to A\to B\to C\to 0$ be a short exact sequence in $\catAbelianGroup$. Then the sequence of constant sheaves, 
    \[0\to \underline{A}\to \underline{B}\to \underline{C}\to 0\] is exact. Indeed, $\left(\underline{Z}\right)_x\cong Z$ for any $x\in X$ and $Z\in \catSet$ (as the fibre of $Z\times X\xrightarrow{\text{pr}_2} X$ at $x\in X$). 
\end{exmp}
